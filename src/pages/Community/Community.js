...
const Community = () => {
  ...
  const fetchRandomNonLikedClothing = (type) => {
    const userId = 'yourUserId'; // Reemplaza esto con la lógica para obtener el userId
    axios.get(`https://clothcraft.azurewebsites.net/clothing/randomNonLiked/byType/${type}`, {
  ...
  const handleSmash = async () => {
    ...
    try {
      const likeResponse = await axios.post(`https://clothcraft.azurewebsites.net/likeClothing`, {
